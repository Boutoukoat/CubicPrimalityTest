...
// sieve small primes
static sieve_t uint64_prime_sieve(uint64_t a)
{
	if (a <= 152) {
		...
		sieve_t stooopid_prime_table[] = {
			PRIME_FOR_SURE, PRIME_FOR_SURE, PRIME_FOR_SURE, PRIME_FOR_SURE, COMPOSITE_FOR_SURE, PRIME_FOR_SURE,
			COMPOSITE_FOR_SURE, PRIME_FOR_SURE, COMPOSITE_FOR_SURE, COMPOSITE_FOR_SURE, COMPOSITE_FOR_SURE,
			PRIME_FOR_SURE, COMPOSITE_FOR_SURE, PRIME_FOR_SURE, COMPOSITE_FOR_SURE,
...
void cubic_primality_self_test(void)
{
	...
	printf("Sieve ...\n");
	assert(uint64_prime_sieve(101) == PRIME_FOR_SURE);
	assert(uint64_prime_sieve(1661) == COMPOSITE_FOR_SURE);
	assert(uint64_prime_sieve(281474976710677ull) == UNDECIDED);

	mpz_init_set_ui(ma, 1);
	mpz_mul_2exp(ma, ma, 127);
	mpz_sub_ui(ma, ma, 1);
	assert(mpz_prime_sieve(ma) == UNDECIDED);
	mpz_add_ui(ma, ma, 2);
	assert(mpz_prime_sieve(ma) == COMPOSITE_FOR_SURE);
