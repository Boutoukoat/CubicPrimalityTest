...
static uint128_t completed_count = 0;
...
static void set_timeout(void)
{
    ...
    while (tail < head)
    {
	    ...
	    if (progress[i].state != STATE_DONE) break;
	    tail++;
    }
    ...
    while (1)
    {
        ...
        if (rc > 0)
        {
			...
			{	
			    ...
			    current_t = __rdtsc();
                            progress[head].seed = v_seed;
                            progress[head].count = v_count;
                            progress[head].t_start = current_t;
                            progress[head].expected_t_end = v_count * connections[cid].rate + current_t;
                            progress[head].cid = cid;
                            progress[head].state = STATE_RUNNING;
                            connections[cid].state = STATE_RUNNING;
                            connections[cid].progress = head;
                            rc = tlv_write(i, cid, TLV_SEED, progress[head].seed);
                            rc = rc < 0 ? rc : tlv_write(i, cid, TLV_COUNT, progress[head].count);
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < (long)argc; i++)
    {
        ...
        if (!strcmp(argv[i], "-e"))
        {
            scan128(argv[++i], &completed_count);
	    lll.set_seed(INIT_SEED);
	    lll.get_seed(completed_count);
            continue;
        }
